...
function renderMap() {
	...
	google.maps.event.addListener(myLoc, 'click', function() {
		infowindow.setContent(myLoc.title);
...
function makeMapListener(window, map, marker) {
	return function() {
		...
		for (var k = 0; k < scheduleData.schedule.length; k++) {
			...
			//sched += "Train " + train.TripID + " to " + train.Destination + " arriving at:<br />";
			...
			//	sched += pred[l].Stop + " in " + Math.ceil((pred[l].Seconds)/60) + " minutes<br />";
			}
;			//sched += "<br />";
		}
		sched += "</table>";
		//window.setContent("<div>" + marker.title + "<br /><br />" + sched + "</div>");
